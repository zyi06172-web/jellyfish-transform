...
旧时间线视频编辑器已删除。视频节点、关键帧节点和分镜表节点负责提供本地下载入口：单个产物直接打开下载，多产物打包为 zip，供外部剪辑软件继续处理。画布顶部提供三个素材操作：按项目比例生成前 5 个内容格的关键帧任务、源码排版导出 6 格分镜表 PNG、在用户确认后调用 Seedance 视频任务接口。
