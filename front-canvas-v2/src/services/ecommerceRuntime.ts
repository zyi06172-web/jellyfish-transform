...
export async function generateProductHero(propId: string, productName: string, description: string, refFileId?: string) {
  ...
  return { fileId: done1?.file_id ?? undefined, url: done1?.file_id ? buildFileDownloadUrl(done1.file_id) : await fetchImageUrlFromTaskResult(taskId) }
}
...
export async function generateProductMultiAngle(propId: string, productName: string, description: string) {
  ...
  for (const slot of targets) {
    ...
    results[slot.view_angle] = { url: row?.file_id ? buildFileDownloadUrl(row.file_id) : await fetchImageUrlFromTaskResult(taskId) }
  }
  return results
}
