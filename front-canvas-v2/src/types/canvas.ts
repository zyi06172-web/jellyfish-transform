...
/** 资源类节点（§8 真实化）：便签 / 上传素材 */
export type ResourceNodeKind = 'sticky' | 'material'
...
/** 电商 workflow 节点（§4.2）：商品节点 + 通用图片节点承载各步电商图 */
export type EcommerceNodeKind = 'product'

export type NodeKind = GenericNodeKind | ResourceNodeKind | DramaNodeKind | EcommerceNodeKind
...
export interface BaseNodeData {
  ...
  extracted_entity_type?: 'character' | 'location' | 'prop'
  source_context_node_id?: string
  [key: string]: unknown
}
...
export interface ScriptSceneAnalysis {
  ...
  scene_appearance: string
  environment_mood: string
  psychological_cue: string
...
export const AIGC_IMAGE_PRESETS = [
  ...
  { key: 'storyboard_page', label: '手绘故事板' },
  { key: 'hand_drawn_storyboard', label: '手绘分镜表' },
  { key: 'shot_script', label: '分镜脚本生成' },
] as const
