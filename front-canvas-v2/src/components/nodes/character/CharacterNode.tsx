...
function CharacterNodeInner({ id, data, selected }: NodeProps) {
  ...
  return (
    <NodeChrome
      ...
      status={images.length ? 'ready' : d.extracted_description ? 'empty' : 'loading'}
      selected={selected}
      compact={compact}
      data={d}
      ...
      {images.length ? (
        ...
        <div className="nw-node-empty">
          {d.extracted_description ? (
            <AssetQuickGenerate nodeId={id} kind="character" name={d.name} description={d.extracted_description} />
          ) : (
            <>
              <UserOutlined style={{ fontSize: 32 }} />
              <span>等待 agent 对话生成四视图…</span>
              <button className="nw-btn nw-btn-secondary" onClick={() => setAgentOpen(true)}>
                去对话框查看进度
              </button>
            </>
          )}
